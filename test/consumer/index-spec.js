...
describe('Consumer tests', function () {
  ...
  const smallQueueSizeConsumer = new Consumer({
    discoveryServers,
    jwt,
    topic: 'persistent://public/default/test',
    subscription: 'subscription',
    subType: Consumer.SUB_TYPES.FAILOVER,
    consumerName: 'SmallQSizeCons',
    readCompacted: false,
    receiveQueueSize: 5,
    logLevel: LEVELS.INFO,
  });
  ...
  afterEach(async function () {
    ...
    if (smallQueueSizeConsumer._isSubscribed) {
      await smallQueueSizeConsumer.unsubscribe();
    }
  ...
  //   it('Should consume all the messages that are produced over time', async function () {
  //     try {
  //       let messageCounter = 0;
  //       const numberOfMessages = 1000;
  //       const messages = Array(numberOfMessages).fill('message');
  //       await smallQueueSizeConsumer.subscribe();
  //       await utils.produceMessages({ messages });
  //       await new Promise((resolve, reject) => {
  //         smallQueueSizeConsumer.run({
  //           onMessage: async () => {
  //             messageCounter++;
  //             console.log(messageCounter);
  //             messageCounter === numberOfMessages && resolve();
  //           },
  //         });
  //       });
  //     } catch (e) {
  ...
  describe('Redeliver unacknowledged messages', function () {
    ...
    it('Should read the batch of the unacknowledged message again before the rest of the flow, in batch, in shared subscription', async function () {
      ...
      await producer.close();
      assert.deepEqual(receivedMessages, ['first', 'first', 'second', 'third', 'second', 'third']);
    });
  });
});
